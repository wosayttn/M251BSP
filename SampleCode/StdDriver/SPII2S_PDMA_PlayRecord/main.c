...
volatile uint8_t gu8Count = 0;
volatile uint8_t gu8TxIdx = 0, gu8RxIdx = 0;
volatile uint32_t u32PlayReady = 0, u32RecReady = 0;

DSCT_T g_asDescTable_TX[2] = {0}, g_asDescTable_RX[2] = {0};
...
{
    g_asDescTable_TX[id].CTL =
        (g_asDescTable_TX[id].CTL & ~(PDMA_DSCT_CTL_TXCNT_Msk | PDMA_DSCT_CTL_OPMODE_Msk))
        | ((BUFF_LEN - 1) << PDMA_DSCT_CTL_TXCNT_Pos)
        | PDMA_OP_SCATTER;
}
...
{
    g_asDescTable_RX[id].CTL =
        (g_asDescTable_RX[id].CTL & ~(PDMA_DSCT_CTL_TXCNT_Msk | PDMA_DSCT_CTL_OPMODE_Msk))
        | ((BUFF_LEN - 1) << PDMA_DSCT_CTL_TXCNT_Pos)
        | PDMA_OP_SCATTER;
}
...
void PDMA_IRQHandler(void)
{
    ...
    volatile uint32_t u32Status = PDMA_GET_INT_STATUS(PDMA);
    ...
    {
        if (PDMA_GET_TD_STS(PDMA) & (1 << I2S_TX_DMA_CH))            /* channel 1 done */
        {
            /* Reset PDMA Scater-Gatter table */
            PDMA_ResetTxSGTable(gu8TxIdx);
            gu8TxIdx ^= 1;
            u32PlayReady = 1;
            PDMA_CLR_TD_FLAG(PDMA, PDMA_TDSTS_TDIF0_Msk);
        }

        if (PDMA_GET_TD_STS(PDMA) & (1 << I2S_RX_DMA_CH))            /* channel 2 done */
        {
            if (gu8Count == 0)
            {
...
int32_t main(void)
{
    ...
    for (u32DataCount = 0; u32DataCount < BUFF_LEN; u32DataCount++)
    {
        PcmRxBuff[0][u32DataCount] = 0;
        PcmRxBuff[1][u32DataCount] = 0;
    ...
    g_asDescTable_TX[0].NEXT = (uint32_t)&g_asDescTable_TX[1];
    ...
    g_asDescTable_TX[1].NEXT = (uint32_t)&g_asDescTable_TX[0];   //link to first description
    ...
    g_asDescTable_RX[0].NEXT = (uint32_t)&g_asDescTable_RX[1];
    ...
    g_asDescTable_RX[1].NEXT = (uint32_t)&g_asDescTable_RX[0];   //link to first description
