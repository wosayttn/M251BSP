...
void SendChar_ToUART(int ch)
{
    while (DEBUG_PORT->STATUS & SC_STATUS_TXFULL_Msk) {}
...
void SendChar_ToUART(int ch)
{
    ...
    do
    {
        ...
        if ((DEBUG_PORT->STATUS & SC_STATUS_TXFULL_Msk) == 0)
        {
...
char GetChar(void)
{
    ...
    else
    {
        ...
        {
            if ((DEBUG_PORT->STATUS & SC_STATUS_RXEMPTY_Msk) == 0U)
            {
...
int kbhit(void)
{
    return !((DEBUG_PORT->STATUS & SC_STATUS_RXEMPTY_Msk) == 0U);
}
...
int IsDebugFifoEmpty(void)
{
    return ((DEBUG_PORT->STATUS & SC_STATUS_TXEMPTY_Msk) != 0U);
}
...
int _read(int fd, char *ptr, int len)
{
    while ((DEBUG_PORT->STATUS & SC_STATUS_RXEMPTY_Msk) != 0);
