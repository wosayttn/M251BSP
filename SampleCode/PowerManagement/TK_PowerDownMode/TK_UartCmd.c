...
#define E_CMD_TYPE2_GET_RUNTIME_TK_PRESSED    ('q')
    ...
    volatile uint8_t g_bIsFineTuneDone = 0;
    volatile uint8_t g_FineTuneDoneTimeOut = 0;
#endif
...
uint8_t g_u8TXBuf[TXBUFSIZE];
uint8_t g_u8RXBuf[RXBUFSIZE];
...
int8_t TK_CmdType2(uint8_t *pu8RXBuf)
{
    ...
    switch (*pu8RXBuf)
    {
        case E_CMD_TYPE2_READ_CHANNEL_MSK:
            g_u8TXBuf[0] = psTkInfo->u32EnChanMsk;
            g_u8TXBuf[1] = psTkInfo->u32EnChanMsk >> 8;
            g_u8TXBuf[2] = psTkInfo->u32EnChanMsk >> 16;
            g_u8TXBuf[3] = psTkInfo->u32EnChanMsk >> 24;

            break;

        case E_CMD_TYPE2_READ_SLIDER_MSK:
            g_u8TXBuf[0] = psTkInfo->u32EnSliderMsk;
            g_u8TXBuf[1] = psTkInfo->u32EnSliderMsk >> 8;
            g_u8TXBuf[2] = psTkInfo->u32EnSliderMsk >> 16;
            g_u8TXBuf[3] = psTkInfo->u32EnSliderMsk >> 24;
            break;

        case E_CMD_TYPE2_READ_WHEEL_MSK:
            g_u8TXBuf[0] = psTkInfo->u32EnWheelMsk;
            g_u8TXBuf[1] = psTkInfo->u32EnWheelMsk >> 8;
            g_u8TXBuf[2] = psTkInfo->u32EnWheelMsk >> 16;
            g_u8TXBuf[3] = psTkInfo->u32EnWheelMsk >> 24;
            //g_u8TXBuf[4] = 'A';
            break;

        case E_CMD_TYPE2_READ_REF_SHIELD_NO:
            g_u8TXBuf[0] = psTkInfo->u8RefChan;
            g_u8TXBuf[1] = psTkInfo->u8ShieldChan;

            if (pu8RXBuf[1] == 0)
            {
                g_u8TXBuf[2] = psTkInfo->u8SliderRes;
                g_u8TXBuf[3] = i8SliderPercentage;
            }
            else
            {
                g_u8TXBuf[2] = psTkInfo->u8WheelRes;
                g_u8TXBuf[3] = i8WheelPercentage;
            }

            break;

        case E_CMD_TYPE2_READ_FEATURE:
            g_u8TXBuf[0] = psTkFeat->u8EnIIR;
            g_u8TXBuf[1] = psTkFeat->u8EnDebounce;
            g_u8TXBuf[2] = psTkFeat->u8EnBaseLineTrace;
            break;

        case E_CMD_TYPE2_READ_SENSE:
            g_u8TXBuf[0] = psTkInfo->u8SenPulse;
            g_u8TXBuf[1] = psTkInfo->u8SenTimes;
            g_u8TXBuf[2] = psTkInfo->u8PolarityCapacitorBank;
            break;

        case E_CMD_TYPE2_READ_RESET_VOL:
            g_u8TXBuf[0] = psTkFeat->u16ResetTime;
            g_u8TXBuf[1] = psTkFeat->u16ResetTime >> 8;
            g_u8TXBuf[2] = psTkInfo->u8AvcchLevel;
            break;

        case E_CMD_TYPE2_READ_STORE_ADDR:
            g_u8TXBuf[0] = *((uint8_t *)&psTkInfo->u32StoreAddr);
            g_u8TXBuf[1] = *((uint8_t *)&psTkInfo->u32StoreAddr + 1);
            g_u8TXBuf[2] = *((uint8_t *)&psTkInfo->u32StoreAddr + 2);
            g_u8TXBuf[3] = *((uint8_t *)&psTkInfo->u32StoreAddr + 3);
            break;

        case E_CMD_TYPE2_READ_NEIGHBORING:
#ifdef OPT_NEIGHBOR
            g_u8TXBuf[0] = psKeyNeighbor[pu8RXBuf[1]].left;
            g_u8TXBuf[1] = psKeyNeighbor[pu8RXBuf[1]].right;
#endif
            break;

        case E_CMD_TYPE2_READ_CHANNEL_PIN:
            g_u8TXBuf[0] = (((pu8RXBuf[1] >= 16) ? psTkFeat->u32PinSel1 : psTkFeat->u32PinSel) >> ((pu8RXBuf[1] % 16) * 2)) & 0x3;
            break;
        ...
        case E_CMD_TYPE2_GET_CALIBRATION1:
            ...
            {
                chan &= ~0x80;
                g_u8TXBuf[0] = inp8(u32KeyStoreAddr + TK_CCB_OFFSET + chan * 4);
                g_u8TXBuf[1] = inp8(u32KeyStoreAddr + TK_REFCB_OFFSET + chan * 4);
            }
            else
            {
                g_u8TXBuf[0] = (psKeyInfo + chan)->ccb;
                g_u8TXBuf[1] = (psKeyInfo + chan)->refcb;
            }
        ...
        case E_CMD_TYPE2_GET_CALIBRATION2:
            ...
            else
            {
                ...
                g_u8TXBuf[0] = (psKeyInfo + chan)->level;
                g_u8TXBuf[1] = (psKeyInfo + chan)->threshold;
            }
        ...
        case E_CMD_TYPE2_GET_IIR_PARAMETER:
            g_u8TXBuf[0] = psTkFeat->u8IIRNew;
            g_u8TXBuf[1] = psTkFeat->u8IIROld;
            break;

        case E_CMD_TYPE2_GET_DEBOUNCE_PARAMETER:
            g_u8TXBuf[0] = psTkFeat->u8Entry;
            g_u8TXBuf[1] = psTkFeat->u8Escape;
            break;
        ...
        case E_CMD_TYPE2_EXPORT_CALIBRATION:
            ...
            g_u8TXBuf[0] = inp8(u32KeyStoreAddr);
            g_u8TXBuf[1] = inp8(u32KeyStoreAddr + 1);
            g_u8TXBuf[2] = inp8(u32KeyStoreAddr + 2);
            g_u8TXBuf[3] = inp8(u32KeyStoreAddr + 3);
            break;

#if defined(MASS_FINETUNE)

        case E_CMD_TYPE2_GET_UID:
            g_u8TXBuf[0] = (USER_ID & 0xFF000000) >> 24;
            g_u8TXBuf[1] = (USER_ID & 0xFF0000) >> 16;
            g_u8TXBuf[2] = (USER_ID & 0xFF00) >> 8;
            g_u8TXBuf[3] = (USER_ID & 0xFF);
            break;
        ...
        case E_CMD_TYPE2_GET_RUNTIME_TK_PRESSED:
            ...
            g_u8TXBuf[0] = asPressedTouchKeys[u8Group].u8TKChanNum;

            if (asPressedTouchKeys[u8Group].i8Count >= 2)
                g_u8TXBuf[1] = 2;
            else
                g_u8TXBuf[1] = asPressedTouchKeys[u8Group].i8Count;

            g_u8TXBuf[2] = asPressedTouchKeys[u8Group + 1].u8TKChanNum;

            if (asPressedTouchKeys[u8Group + 1].i8Count >= 2)
                g_u8TXBuf[3] = 2;
            else
                g_u8TXBuf[3] = asPressedTouchKeys[u8Group + 1].i8Count;
        ...
        case E_CMD_TYPE2_FIRMWARE_VERSION:
            if (pu8RXBuf[1] == 0)
            {
                g_u8TXBuf[0] = FIRMWARE_MAJOR_VERSION;
                g_u8TXBuf[1] = FIRMWARE_MINOR_VERSION;
                g_u8TXBuf[2] = TOUCHKEY_VERSION;
            }
            else if (pu8RXBuf[1] == 1)
            {
                g_u8TXBuf[0] = TKLIB_MAJOR_VERSION;
                g_u8TXBuf[1] = TKLIB_MINOR_VERSION;
                g_u8TXBuf[2] = TOUCHKEY_VERSION;
            }
            else
            {
                //for PDID
                u32PDID = SYS->PDID;
                g_u8TXBuf[0] = (uint8_t)((u32PDID & 0xFF000000) >> (24));
                g_u8TXBuf[1] = (uint8_t)((u32PDID & 0x00FF0000) >> (16));
                g_u8TXBuf[2] = (uint8_t)((u32PDID & 0x0000FF00) >> (8));
                g_u8TXBuf[3] = (uint8_t)((u32PDID & 0x000000FF));
            }
...
void UART0_IRQHandler(void)
{
    ...
    {
        g_u8RXBuf[gu8comRtail] = UART_READ(TK_UART_PORT);
        UART_ClearIntFlag(TK_UART_PORT, UART_INTSTS_RDAINT_Msk);
        gu8ChkSum += g_u8RXBuf[gu8comRtail];
        gu8comRtail = gu8comRtail + 1;
    }

    if (g_u8RXBuf[0] <= E_CMD_TYPE1_NUM_LAST_COMMAND)
    {
        u8CmdType = 1;
    }
    else if ((g_u8RXBuf[0] >= 'A') && (g_u8RXBuf[0] <= E_CMD_TYPE1_LAST_COMMAND))
    {
        u8CmdType = 1;
    }
    else
    ...
    if ((g_u8RXBuf[0] == E_CMD_TYPE2_CHECK_CALIBRATION_DONE) &&
            (gu8CmdSts == E_CMD_TYPE2_READY))
    {
        ...
        else
        {
            ...
            if (g_bIsFineTuneDone == 1)
            {
                u8Arry[0]  = 1;                      /* done    */

                if (g_FineTuneDoneTimeOut == 1)
    ...
    if ((g_u8RXBuf[0] == E_CMD_TYPE2_RUNTIME_KEY_INFO) &&
            (gu8CmdSts == E_CMD_TYPE2_READY))  /* Run time get Touch data */
    {

        uint8_t i;
        uint8_t u8Arry[5] = {0};
        uint8_t ch = g_u8RXBuf[1];
